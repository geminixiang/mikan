...
`cdp.mjs` in the skill directory (one level above this reference) is the driver. It is dependency-free on purpose —
...
Run driver commands from the skill directory, not `references/`.
...
Resolve the conversation's `Office` through the office registry (`src/office/README.md`); do not construct paths from raw conversation IDs. Relevant evidence includes:
...
These command names and the leading-space workaround are historical observations, not current command contracts. Check `src/commands/manifest.ts` and current parsing before using them. Slack's slash-menu suggestions are not proof that mikan supports a command.
...
### 12. An unresolved model can resemble an intake failure
...
If output stops short, compare model output with delivered output before attributing the difference to platform truncation.
