...
- **Wire conversation identity**: The opaque platform-issued identifier used only to address that platform's API. It is not globally unique and must not be used alone as a filesystem, session, credential, runtime, lease, or placement identity.
- **Conversation storage scope**: The durable identity of one conversation inside a Workspace, composed from the platform namespace and wire conversation identity. Every persisted or runtime-owned conversation resource uses this scope; the Workspace itself supplies the outer namespace through its host workspace root.
- **Legacy conversation storage**: Conversation state stored directly under a wire conversation identity before conversation storage scopes existed. It may move to a scoped identity only when its platform owner is uniquely proven; ambiguous ownership fails closed rather than assigning history or runtime authority to whichever platform starts first.
- **Session key**: The conversation-scoped runtime identity used to serialize and resume work for a direct message, shared channel, or thread. A session key belongs to exactly one conversation storage scope: either the scope's conversation identity or that identity plus an opaque scoped suffix. Platform-provided session keys must never select another conversation's runtime state.
