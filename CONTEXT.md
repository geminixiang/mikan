...
- **Capability portal**: A limited-purpose server-rendered surface opened through a short-lived bearer link (`/session`, `/admin`, or `/link`). A portal token grants only that portal's authority; it is not website identity and never enters the Harness Web Client.
- **Harness Web Client**: The authenticated browser product for creating and driving Web Conversations. It owns route selection, drafts, and live projection; the daemon owns offices, durable sessions, runs, models, and event order.
- **Web Conversation**: A first-class Conversation office created by the Harness Web Client with `platform = web`. It belongs to one immutable OAuth principal and never aliases the platform office used to admit that principal.
- **Browser session**: A short-lived httpOnly cookie proving an admitted OAuth principal to the Harness Web Host. It grants access only to that principal's Web Conversations, not to Admin, vault, or Session View capabilities.
