...
- `index.ts`: The adapter contract — re-exports the platform-neutral `MessagingBot`, `ConversationEvent`, `ConversationMessage`, `ConversationResponder`, and related interfaces from `src/types.ts`, and owns `createConversationEvent` / `createConversationMessage`, which normalize any intake (platform or scheduler) onto one authoritative `OfficeAddress`.
