...
- `jev-context.ts`: Reads the recent messages of one scope (channel top level or one thread) from the office log for Jev decisions, merging streamed bot chunks.
- `auto-reply-context.ts`: The `jev` auto-reply question and its state (recent scope plus whether mikan already replied in the thread).
- `task-intent.ts`: Classifies a DM message around a task as `status` / `steer` / `request` with Jev, falling back to `isTaskStatusQuestion` when Jev is unavailable.
- `task-status.ts`: Task-root discovery from the office log, `task_status` observations, the regex status shortcut, and status formatting.
