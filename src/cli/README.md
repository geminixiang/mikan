...
- `ext.ts`: `mikan ext install|validate|list|remove` — manages extensions from the CLI; installs into the host-only state dir. Git sources materialize through `packages/` (cached clone under the scope's `git/` dir, `@ref` + `#subpath` supported).
