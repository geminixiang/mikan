...
- `boot.ts`: Pure argv → `BootPlan` resolution — which mode to run (`ext`, `env`, `help`, `version`, `worker-token`, `onboard`, `download`, `run`) and with what configuration — plus the `--help` text; `main.ts` executes the plan.
