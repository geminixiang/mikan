...
Shared profiles are operator-managed credentials and apply only through an explicit `/login copy <name>`; the retired `sandbox.defaultSharedVault` no longer copies them into new conversations.
