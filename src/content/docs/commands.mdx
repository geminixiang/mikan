...
| `/model` / `/pi-model [provider/model[:thinking]]` | Show or switch the current conversation model. Thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`. |
...
<Aside type="note" title="Platform command registration">
  Native slash commands must also be registered with the platform. Slack's example manifest includes
  `/pi-login`, `/pi-session`, `/pi-new`, `/pi-model`, `/pi-auto-reply`, `/pi-sandbox`,
  `/pi-extensions`, and `/pi-admin`. Discord registers its application commands at startup. Telegram
  uses bot commands handled through long polling.
</Aside>
