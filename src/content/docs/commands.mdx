...
| `/pi-sandbox door <default\|isolated\|shared\|shared-private\|full>` | Set this office's door policy. See [Office door policy](#office-door-policy) below.                                        |
...
| Word             | Effect                                                                                                           |
| ---------------- | ---------------------------------------------------------------------------------------------------------------- |
| `default`        | Clear the override; follow global `sandbox.workspace`, or platform visibility when the global setting is absent. |
| `isolated`       | Mount only this office's own directory.                                                                          |
| `shared`         | Trusted door: the office plus read-write workspace-level `MEMORY.md`, `skills/`, and `events/`.                  |
| `shared-private` | Trusted door: the same shared-support layout, but workspace-level `MEMORY.md` is read-only.                      |
