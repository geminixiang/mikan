...
Slack auto-reply is a per-conversation boolean stored as `slack.autoReply` and changed with `/pi-auto-reply on|off`. When enabled, unaddressed top-level human messages in that Slack channel trigger mikan. Legacy marker files, rules, top-level `autoReply`, and `llm.autoReply` judge-model settings remain retired and are ignored.
