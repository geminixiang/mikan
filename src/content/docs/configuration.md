...
| `llm.image.provider`           | unset               | Optional provider for the `generate_image` tool; defaults to `llm.provider`                                                                                                                                                  |
| `llm.image.model`              | unset               | Optional model id sent to the provider's `/images/generations` endpoint; defaults to `llm.model`. May name an image-only deployment that is not a registered chat model — it borrows the provider's endpoint and credentials |
...
`/pi-model` writes a partial conversation override. Office door policy is an Admin/host setting rather than a chat command; `/pi-sandbox` reports it and manages resource boost only. Auto-reply enablement and rule text are managed by `/pi-auto-reply` and the conversation's `auto-reply` marker file, not by the JSON settings fields.
