...
The Gondolin worker/gateway path is a distinct wire protocol, not an in-process sandbox implementation. Its contracts live in `gondolin-contract.ts`, `gondolin-worker-client.ts`, gateway/join modules, inventory, placement, and fleet code.
...
For the detailed deployment contract, see [Gondolin remote worker](/sandbox/gondolin-remote-worker/).
