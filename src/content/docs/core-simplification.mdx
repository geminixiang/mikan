...
8. registries for optional platform, command, tool, and sandbox capabilities.

Web portals, particular chat SDKs, Docker provisioning, Cloudflare execution, and GitHub-specific tools are products or plugins around this core.
...
| P1       | The harness is both a public SDK and an internal engine         | Root exports session parsing, credentials, settings, event schema, and runner details together                                                              | Publish `./runtime` and optionally `./harness` subpaths with separate stability policies                             |
| P2       | Optional runtime products are compiled into the central CLI     | Platform SDKs, portals, and four sandbox modes are assembled in `main.ts`                                                                                   | Make `main.ts` a composition root over registries; lazy/optional product modules become replaceable                  |
...
This removes feature detection by arbitrary method name and lets commands request the exact capability they need.
...
The default CLI registers stable backends. Experimental backends such as Cloudflare can move behind optional product modules without changing tool or runtime code.
...
3. Pass narrow capabilities to commands, events, and admin instead of the whole bot.
4. Retire the now-unused `ChatAdapter` shallow interface. (Completed.)
...
3. Register experimental backends such as Cloudflare from optional composition modules.
