...
<Aside type="note" title="Graceful reload">
  ...
  aborts the rest and posts a restart notice in those conversations, and gives Sentry up to five
  seconds to flush. `pm2 reload` in fork mode is sequential, so the drain is also a window with no
  process connected to the platforms; deploy off-peak.
</Aside>
