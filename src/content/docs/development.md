...
To develop against the Open Connector gateway (`connector_gws` / `connector_github` tools), see the [local development section of the connector guide](/connector/) — `pm2 start ecosystem.config.cjs --only open-connector` plus three `CONNECTOR_*` env vars on the line above.
