...
Runs default to 100 model turns, 10 minutes, and USD 10. A named `profile` may impose stricter caps; request budgets can only tighten those caps, never raise them. Built-in profiles can be patched by `<workspaceDir>/agents/<name>.md`, while new profile files must declare their tool grant.
