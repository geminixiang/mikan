...
`api.fetchHistory` reads recent top-level messages (oldest first, one page — pass the last `ts` as `oldest` to page forward) and `api.listUsers` lists the workspace's active users, both without an agent run. Slack backs them with `conversations.history` and `users.list`; platforms without the capability throw a descriptive error.
