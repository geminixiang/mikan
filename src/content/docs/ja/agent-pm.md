...
:::caution[Slack ではスラッシュを外して `pm status`]
Slack のクライアントは `/` で始まる行をすべて Slack App 自身に宣言されたコマンドのために
取っておくので、`/pm` はあなたの端末から出ていきません —— そして extension がそこに宣言
する手段もありません。そこで Slack は名前だけを受け付けます。このページのコマンドは
Slack ではそう打ちます。

それ以外 —— Telegram、Discord、GitHub のコメント —— ではスラッシュが必要で、省くと静かに
失敗します。テキストは agent に届き、agent はもっともらしい依頼だと見て推測し、「未完了の
task はありません」のような答えを返します。正しそうに見え、model 呼び出しを 1 回消費し、
pipeline については何も伝えていません。

どちらでも、コマンドの出力は `agent-pm — <日付>` で始まります。その行がなければ、いま
話していた相手は agent です。
:::
...
| pipeline を操作する —— 状態確認、即時実行     | `/pm …`（Slack では `pm …`）、model なし |
...
Slack では、これらはすべて先頭のスラッシュなしで打ちます: `pm all`、`pm run`。
