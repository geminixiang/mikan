...
:::caution[スラッシュは省略できず、省くと静かに失敗します]
`pm status` と打ってもコマンドは実行されません。agent に届き、agent はもっともらしい依頼
だと見て推測し、「未完了の task はありません」のような答えを返します —— 正しそうに見え、
model 呼び出しを 1 回消費し、pipeline については何も伝えていません。コマンドの出力は必ず
`agent-pm — <日付>` で始まります。その行がなければコマンドは動いていません。
:::
