...
OpenConnector は起動時の deployment-owned な完全な `OPENCONNECTOR_ENDPOINT` と host-only の `OPENCONNECTOR_ADMIN_TOKEN` だけで設定し、Admin Marketplace には表示しません。mikan が予約名 `open-connector` の server を注入し、global または conversation の `mcpServers` 設定で置換や無効化はできません。Provider OAuth connections を共有したまま Slack Conversation office ごとに個別の runtime identity を持ち、admin credential を送信できるのは endpoint の origin だけです。Office の runner を初めて作成するとき、mikan は `mikan:slack:<workspace-id>:<channel-id>` という名前の token を作成し、現在の OpenConnector deployment の action／proxy policy をコピーして、その office の private State-dir に保存します。Provisioning の失敗はその runner の OpenConnector だけを無効にします。Managed sandbox に admin token や runtime token は渡されません。Host sandbox にはこの分離がないため trusted として扱う必要があります。
