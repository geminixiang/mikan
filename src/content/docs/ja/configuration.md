...
Admin の MCP panel には repository-owned の curated Marketplace があります。install 前に完全な host command または remote endpoint、必要な credentials、source、target scope、security warning を表示し、確認後は通常の `mcpServers` entry だけを書き込みます。Local package version は pin され、別の installed database や automatic updater は作りません。また、catalog 掲載は security certification ではありません。Local stdio preset は mikan host 上で code を実行し、remote preset はその tool に送られた call と data を受信します。OpenConnector preset は `http://127.0.0.1:3737/mcp` の self-hosted sibling runtime に接続します。full-access bootstrap token ではなく、scope を限定した persistent runtime token を install してください。Provider credentials は OpenConnector に保持され、Sandbox Vault には入りません。

Deployment-owned の `OPENCONNECTOR_ORIGIN` と host-only の `OPENCONNECTOR_ADMIN_TOKEN` 環境変数を設定すると、provider OAuth connections を共有したまま、Slack Conversation office ごとに個別の OpenConnector runtime identity が割り当てられます。MCP endpoint はこの固定 origin と一致する必要があるため、conversation override が admin credential を別の host に転送することはできません。Office の runner を初めて作成するとき、mikan は `mikan:slack:<workspace-id>:<channel-id>` という名前の token を作成し、現在の OpenConnector deployment の action／proxy policy をコピーして、その office の private State-dir に保存します。Provisioning の失敗はその runner の OpenConnector だけを無効にします。Managed sandbox に admin token や runtime token は渡されません。Host sandbox にはこの分離がないため trusted として扱う必要があります。
