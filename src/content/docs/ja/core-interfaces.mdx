...
`src/index.ts` が harness 全体を re-export しているため、パッケージは現在このポリシーが意図するよりも多くのシンボルを公開しています。下の表を意図された契約として扱ってください。移行については [コア簡素化](/core-simplification/) を参照してください。
...
runtime は調整役です。platform は harness の永続化を知ってはならず、harness は platform SDK オブジェクトを知ってはならず、tools は実行がローカル、コンテナ化、リモートのいずれであるかを知ってはなりません。
...
`src/` 配下のソースパスや `dist/` 配下の生成パスを import することはサポートされません。パッケージ exports が宣言するパスだけを public とみなすべきです。現時点ではパッケージに明示的な `exports` map がないため、これは強制ではなくドキュメント上の制約です。
...
| フィールド            | 契約                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------- |
| `conversationId`      | 生の platform conversation/channel 識別子。session keys が `:` を予約するため `:` を含んではならない |
...
同じ実行に対する、より豊かな正規化済み message、response port、platform メタデータを保持します:
...
`ConversationMessage` と `ConversationEvent` は現在、message id、session、conversation kind、user、text、attachments、thread identity を重複して持っています。adapters は両方の表現を一貫させ続ける必要があります。これは内部の互換性のための seam であり、新しい integration にとって望ましいモデルではありません。
...
`ChatAdapter` はより小さな lifecycle 専用インターフェース（`start`、`stop`、`getMessagingInfo`）ですが、CLI は `MessagingBot` を使います。呼び出し側が明示的に `ChatAdapter` を要求しない限り、両方を実装しないでください。
...
| メソッド                             | セマンティクス                                                               |
| ------------------------------------ | ---------------------------------------------------------------------------- |
| `handleEvent(event, bot, context)`   | 導出された session key で直列化し、command または agent run を dispatch する |
| `runSession(options)`                | すでに直列化済みの 1 単位を実行する。制御された host 用途を想定              |
| `handleStop(...)` / `forceStop(...)` | 協調的/ユーザーに見える stop か、即時の内部 stop か                          |
...
| メソッド                              | セマンティクス                                                        |
| ------------------------------------- | --------------------------------------------------------------------- |
| `isRunning(sessionKey)`               | 現在のプロセス内での実行状態                                          |
| `getRunningSessions()`                | admin/observability 面向けのスナップショット                          |
| `switchConversationModel(...)`        | cached runner を更新する。存在したかどうかを返す                      |
| `refreshConversationEnvironment(...)` | cached runner の環境を再解決する                                      |
| `shutdown(timeoutMs?)`                | 新しい作業を拒否し、runners を停止し、期限まで in-flight の作業を待つ |

`ConversationRuntimeOptions` は、workspace/configuration フィールド、sandbox/resource サービス、任意の portal token stores、任意の commands、model registry、proactive な platform 操作、platform tool-pack factories を供給します。vault と portal stores が欠けている場合は無効化された実装に degrade します。portal URL を設定しつつその store を設定しない場合、使用時にエラーになります。

並行性の不変条件: 1 つの session key に対する呼び出しは直列で、異なる session keys は並行して実行され得ます。したがって platform tool pack は runner ごとに作成され、決してグローバルに共有されません。`bindRun` がその run binding を変更するためです。
...
### `MikanModels` と `FileCredentialStore`

`MikanModels` は組み込みおよびカスタムの `pi-ai` models を解決します。`FileCredentialStore` は provider credentials を state directory に永続化します。Vault credentials は別の境界です。tool execution に注入されるものであり、host 側の model client を認証するために使われるものではありません。
...
この discriminated union は現在、`host`、`container`、`image`、`gondolin`、`firecracker`、`cloudflare` をサポートします。パース構文と成熟度は [Sandbox](/sandbox/) の下に記載されています。
...
ソース: `src/vault/types.ts`。ポリシーは `src/vault/policy.ts`。
...
## 10. Session、chat、identity のインターフェース

ソース: `src/sessions/*`、`src/store.ts`、`src/sandbox/identity.ts`。

意図的な 2 つの記録があります:

| 記録                              | 目的                                                             |
| --------------------------------- | ---------------------------------------------------------------- |
| `<conversation>/log.jsonl`        | Platform の真実: user/bot の messages と attachments             |
| `<conversation>/sessions/*.jsonl` | Agent の真実: prompts、model/tool messages、branches、compaction |

これらを統合しないでください。Chat history は欠けている top-level agent session を再構築できますが、agent sessions には platform に現れなかったデータが含まれます。

Session keys は `conversationId[:suffix]` を使います。この文法を所有するのは `src/sessions/session-key.ts` だけです。呼び出し側は、文字列を直接分割せず、`deriveSessionKey`、`conversationIdOf`、`threadSuffixOf` を使わなければなりません。
...
| `<state-dir>/conversations/<id>/settings.json`     | config/admin             | Host-only の conversation override      |
| `<state-dir>/auth.json`                            | harness credential store | Host-only の model-provider credentials |
...
| `<workspace>/<id>/log.jsonl`                       | platform store           | Append-only な platform history         |
| `<workspace>/<id>/sessions/*.jsonl`                | session store            | Versioned append-only な agent history  |
...
Gondolin の worker/gateway パスは、in-process の sandbox 実装ではなく、独立した wire protocol です。その契約は `gondolin-contract.ts`、`gondolin-worker-client.ts`、gateway/join modules、inventory、placement、fleet code に存在します。

安定した概念は、runtime spec と handle identity、worker registration/heartbeat、leased placement、exec sessions、file transport、runtime lifecycle です。Lease watermarks は failover の間、古い workers を fence します。Wire の変更は、controller、gateway、workers の間でバージョン管理するか、互換にデプロイしなければなりません。

詳細なデプロイ契約については [Gondolin remote worker](/sandbox/gondolin-remote-worker/) を参照してください。
