---
title: コアインターフェース
description: mikan の platform、runtime、harness、session、execution、vault、package、host の各レイヤー間でサポートされる境界。
---
...
2. execution resolver が Workspace projection と packages を一度だけ解決し、actor/vault identity を選択する。
3. `VaultManager` が env と mounts を解決し、collision と backend capability checks が grant 全体を検証する。
4. 1 つの execution decision が projection、packages、runtime path context、およびその run 用 credentials だけを持つ concrete executor を返す。
