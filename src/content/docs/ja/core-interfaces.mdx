...
Office key は**逆変換できません**。`OfficeRegistry`（`office-registry.json`、host 専用）が生 id ↔ office の恒久的な対応であり、旧来の生 id 配置からの boot 時 migration も journal します。生 id を扱う面はこれを介して解決します: CLI operator 向けの `resolveOwnedOfficeAddress`、admin の列挙向けの `listRegisteredOffices`。sandbox のリソース名（container 名、Gondolin instance、Cloudflare scope）は、リソース名の migration が済むまで生の conversation 由来のままです。そこでの衝突のコストは container の作り直しであって、認証情報へのアクセスではありません。
