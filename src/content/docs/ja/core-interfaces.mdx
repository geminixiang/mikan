...
`gondolin:default` は mikan process が conversation ごとにローカル VM を作成・所有します。command ごとに独立した Gondolin session IPC connection を使うため、abort/timeout は guest command を終了します。runtime は process lifecycle のみ存続し、通常の shutdown は projection を同期して VM を閉じ、restart 後は cold start します。network worker protocol や remote placement contract はありません。
