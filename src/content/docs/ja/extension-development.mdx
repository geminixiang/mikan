...
投稿時、各 `action_id` には `ext:<slug>:…` namespace が付きます。そのため操作はこの extension の `onAction` handler だけに渡され、agent run は開始されません。処理後は message を更新して button を削除または無効化してください。完全な例は `examples/extensions/poll` にあります。
