...
実行の既定値はモデルターン 100 回、10 分、USD 10 です。名前付き `profile` はより厳しい上限を設定でき、request budget はその上限を引き下げることだけができ、引き上げることはできません。組み込み profile は `<workspaceDir>/agents/<name>.md` で調整でき、新しい profile file は tool grant を宣言する必要があります。
