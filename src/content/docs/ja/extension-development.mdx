...
ディスパッチはプレーンテキストベースです。受信メッセージのテキストが `/pm` で始まる必要があります。これは `/` で始まるテキストを通常のメッセージとして配信するプラットフォーム——Telegram、Discord、GitHub のコメント——で機能します。Slack クライアントはスラッシュ入力を横取りし、Slack App manifest に登録されたコマンド（mikan はこれを組み込みコマンドへルーティングします）だけを送信するため、現時点では Slack で拡張機能コマンドを `/pm` として呼び出すことはできません。
