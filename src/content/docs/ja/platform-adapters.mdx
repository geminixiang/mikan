...
<CardGrid>
  ...
  <Card title="返信機能" icon="rocket">
    `ChatResponseContext` が返信、更新、typing/working
...
6. `ChatResponseContext` を作成し、返信、更新、typing/working 状態、ファイルアップロードをカプセル化する。
7. イベントを `BotHandler`、つまり `SessionRuntime` に渡す。
...
| ファイル                    | 用途                                                                                           |
| --------------------------- | ---------------------------------------------------------------------------------------------- |
| `src/adapter.ts`            | `Bot`、`BotEvent`、`ChatMessage`、`ChatResponseContext` などの共通 interface を外部公開。      |
...
- `ChatResponseContext`：agent が返信するときに使うチャネル。
