...
目的は、`src/runtime/*` と `src/harness/` が各プラットフォームの SDK、thread ルール、メッセージ更新 API、ファイルダウンロード方式を知らなくてよい状態にすることです。
...
2. そのメッセージが mikan を起動すべきか判定する：DM、mention、thread reply。
...
共通型は `src/adapters/index.ts` から export され、実装側の型は `src/types.ts` と `src/adapters/types.ts` にあります。
...
Slash command の登録とルーティングは、adapter ごとのインベントリではなく `src/adapters/commands/manifest.ts`
...
| ファイル                               | 用途                                                                                                                                 |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `src/adapters/index.ts`                | `MessagingBot`、`ConversationEvent`、`ConversationMessage`、`ConversationResponder` などの共通 interface を外部公開。                |
...
新しいプラットフォームを追加する場合は、まずこれらの抽象を実装し、プラットフォーム SDK の詳細を `src/runtime/*` や `src/harness/` に持ち込まないでください。
