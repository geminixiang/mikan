...
| ファイル                         | 用途                                                                                                                 |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `src/adapters/github/bot.ts`     | GitHub bot 本体：poll loop、watermark dedup、mention/participation triggering、comments。                            |
...
| `src/adapters/github/ids.ts`     | `GH_<owner>_<repo>_<number>` conversation id の encode/parse。                                                       |
...
- `github_checks` tool は push 済み branch（または PR head）の CI check runs を読み取り、失敗した job の log tail を取得できます。これにより agent は CI が通るまで診断と反復を行えます。App permissions **Checks: Read** と **Actions: Read**（logs 用）が必要です。
...
- PR review threads（diff-line comments）は計画されていますが、まだ poll されません。
- `./repo` clone は初回接触時の snapshot です。sandbox は updates を fetch できません。
