...
| 機能                 | 主要コード                                         |
| -------------------- | -------------------------------------------------- |
| Portal HTTP server   | `src/adapters/web/server.ts` の `startWebServer()` |
...
| 共通 token store     | `src/adapters/web/token-store.ts`                  |
| 共通 portal shell    | `src/adapters/web/portal-shell.ts`                 |
