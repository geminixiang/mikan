...
Their goal is to keep `src/runtime/*` and `src/harness/` from knowing each platform's SDK, thread rules, message update API, or file download behavior.
...
2. Decide whether the message should trigger mikan: DM, mention, thread reply.
...
Shared types are exported from `src/adapters/index.ts`; implementation types live in `src/types.ts` and `src/adapters/types.ts`.
...
Slash-command registration and routing derive from `src/adapters/commands/manifest.ts` rather than per-adapter
...
| File                                   | Purpose                                                                                                                         |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `src/adapters/index.ts`                | Exports shared interfaces such as `MessagingBot`, `ConversationEvent`, `ConversationMessage`, and `ConversationResponder`.      |
...
When adding a platform, implement these abstractions first. Do not bring platform SDK details into `src/runtime/*` or `src/harness/`.
