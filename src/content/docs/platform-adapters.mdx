...
<CardGrid>
  <Card title="Event normalization" icon="setting">
    Convert native platform messages, mentions, slash commands, or replies into a shared `BotEvent`.
  </Card>
  ...
  <Card title="Response capability" icon="rocket">
    `PlatformResponder` wraps replies, updates, typing/working state, and file uploads.
  </Card>
</CardGrid>
...
3. Convert it to shared `BotEvent` and `ChatMessage` values.
...
6. Create a `PlatformResponder` that wraps replies, updates, typing/working state, and file uploads.
7. Hand the event to `BotHandler`, which is `ConversationRuntime`.
...
| File                        | Purpose                                                                                               |
| --------------------------- | ----------------------------------------------------------------------------------------------------- |
| `src/adapter.ts`            | Exports shared interfaces such as `Bot`, `BotEvent`, `ChatMessage`, and `PlatformResponder`.          |
...
- `BotEvent`: one user or platform event.
- `Bot`: platform bot capabilities, such as post message and upload file.
- `PlatformEventContext`: `message`, `responder`, and `platform` metadata attached to an event.
- `PlatformResponder`: the channel used by the agent to reply.
