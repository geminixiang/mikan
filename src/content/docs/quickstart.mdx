...
2. Run the interactive setup:
   ...
   Onboarding does not write any office policy: visibility follows the platform conversation type. Telegram private chats are private offices, which only the managed Docker sandbox can enforce; this guide uses `host`, which runs every office in one filesystem and logs a one-time warning per private office instead. That is fine for a first try on your own machine.
...
<Aside type="caution" title="Host sandbox runs on your machine">
  ...
  Docker container and enforces office visibility.
</Aside>
