...
5. Start mikan:
   ...
   The working directory defaults to `~/.mikan/workspace` and is created automatically; pass a path (`mikan --sandbox=host ~/mikan-workspace`) to use a different one. The workspace is where the agent keeps per-conversation files: memory, attachments, session logs, and its working area.
