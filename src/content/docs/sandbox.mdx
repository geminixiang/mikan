...
<CardGrid>
  ...
  <Card title="Distributed Gondolin preview" icon="moon">
    `gondolin:remote` runs Gondolin/QEMU VMs across authenticated workers; `image:*` remains the single-machine managed mode.
  </Card>
</CardGrid>
...
| `gondolin:default`                                          | local Gondolin/QEMU VM         | injected            | generated conversation vault      | Local development/diagnostics profile; requires Node.js >=23.6 and QEMU                                  |
| `gondolin:remote`                                           | Gondolin VM on a remote worker | injected            | generated conversation vault      | Distributed preview; mTLS workers, fenced leases, shared-POSIX workspace transport                       |
...
<LinkCard
  ...
  description="Run the distributed Gondolin worker preview; the local profile is retained for development and diagnostics."
  href="gondolin/"
/>
<LinkCard
