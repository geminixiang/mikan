...
<CardGrid>
  ...
  <Card title="MicroVM preview" icon="moon">
    `microvm:default` runs a local Gondolin/QEMU VM and is the planned successor to managed `image:*` sandboxes.
  </Card>
</CardGrid>
...
| `microvm:default`                                           | local Gondolin/QEMU VM    | not yet supported   | generated conversation vault      | Preview; mounts the current workspace at `/workspace`; requires Node.js >=23.6 and QEMU                  |
...
| Capability                                   | `host`         | `container:<name>`     | `image:<image>` | `microvm:default` | `firecracker:*` | `cloudflare:*`    |
| -------------------------------------------- | -------------- | ---------------------- | --------------- | ----------------- | --------------- | ----------------- |
| command execution                            | ✅             | ✅                     | ✅              | ✅                | ✅              | ✅                |
| mikan-managed runtime lifecycle              | not applicable | ❌                     | ✅              | process lifetime  | ❌              | ❌                |
