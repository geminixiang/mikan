...
`gondolin:remote` runs the same runtimes on a remote Linux/KVM machine through the [mikan-worker daemon](./gondolin-remote-worker/): mutual TLS, fenced leases, and one tunnel per command, with the workspace on shared POSIX storage. The mikan host itself stays on the supported Node floor in this profile — Gondolin only runs on the worker. `image:*` remains available and is still the recommended managed mode while `gondolin:*` is in preview.
