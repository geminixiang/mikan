...
`gondolin:remote` runs the same runtimes on one or more remote Linux/KVM machines through the [mikan-worker daemon](./gondolin-remote-worker/): mutual TLS, fenced leases, one tunnel per command, and sticky per-conversation placement across the fleet (capacity-aware, drainable, with lease-fenced failover), with the workspace on shared POSIX storage. The mikan host itself stays on the supported Node floor in this profile — Gondolin only runs on the workers. The [remote worker quickstart](./gondolin-remote-quickstart/) brings one up end to end (with a one-command smoke test). `image:*` remains available and is still the recommended managed mode while `gondolin:*` is in preview.
