...
`gondolin:default` is local-only: mikan and its detached Node workers run on the same host. `image:*` remains available and is still the recommended managed mode while Gondolin is in preview.
