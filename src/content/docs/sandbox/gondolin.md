...
Each conversation gets a VM created and owned by the mikan process itself. mikan talks to it over Gondolin's session IPC socket, one connection per command, so aborting or timing out a command kills it inside the guest — Gondolin's in-process `vm.exec()` cannot do that, because aborting it only abandons the call and leaves the guest process running. Conversation vault environment variables are sent per command over the (user-only) socket. Commands and file tools run inside that VM with `TZ=Asia/Taipei`.

Which host directories the VM sees follows the office door policy, exactly as in `image:*`: the default `isolated` policy projects only the conversation's own office at `/workspace/<office-key>`; `trusted` / `shared-support` adds the workspace-global `MEMORY.md`, `skills`, and `events`; `trusted` / `full` projects the whole workspace root at `/workspace`. Gondolin is one of the two modes that can enforce the isolated projection — see [Configuration](/configuration/) for the settings and [Sandbox](/sandbox/) for the comparison.
