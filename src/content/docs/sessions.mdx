...
`/new` immediately creates a **Clean session**. It does not summarize the predecessor, invokes Dream, waits for memory processing, or deletes older JSONL files. The existing conversation `MEMORY.md` remains available independently.
