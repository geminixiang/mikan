...
## Bootstrap, Clean sessions, and Dream
...
`/new` immediately creates a **Clean session**. It does not summarize the predecessor, invoke Dream, wait for memory processing, or delete older JSONL files. The existing conversation `MEMORY.md` remains available independently.

**Dream** is system-scheduled office maintenance. During Taiwan time 02:00–05:00, mikan checks each registered Conversation office and processes all newly accumulated settled entries across its current and earlier top-level, thread, and event sessions once the newest evidence has been idle for five hours. A successful run replaces the complete conversation `MEMORY.md` and then advances a host-private per-session checkpoint; without new evidence, no model is called.
