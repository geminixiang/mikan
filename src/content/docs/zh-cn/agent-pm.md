...
:::caution[在 Slack 上要把斜杠去掉：`pm status`]
Slack 客户端会把每一行 `/` 开头的输入留给声明在 Slack App 里的命令，所以 `/pm` 根本
离不开你的电脑 —— 而扩展也没有办法在那里声明。因此 Slack 接受不带斜杠的名字，这页的
命令在 Slack 上就是这样打。

其他地方 —— Telegram、Discord、GitHub comments —— 斜杠不能省，而且省掉之后会安静地
失败：文本会进到 agent，而 agent 看到一个看似合理的请求就会猜，回你类似「无未完成
任务」的答案。看起来像对的，花掉一次模型调用，却完全没告诉你 pipeline 的状况。

两种情况下，命令的输出都以 `agent-pm — <日期>` 开头。没看到那行，就代表你刚刚是在跟
agent 说话。
:::
...
整个循环就是这样：每个台北整点产生一条 `clock.tick` event，心跳 workflow 认领它，然后
发出一条消息 —— 一天一次，因为投递会去重。在 test 模式下，你会在测试对话看到那条消息，
并标注它原本要发去哪里。

:::caution[全新安装不会产生任何 task]
`tasks: 0 open` 不代表哪里坏了，而且它不会自己改变。默认启用的那个 workflow 产生的是
**delivery 而不是 task**，所以在有人新增一个 `creates: "task"` 的 workflow 之前，task
工具没有东西可以列。那是改 extension 的 seeds，不是你能从对话里做的事 —— 下面几节描述的
是「当这种 workflow 存在之后」会怎么运作。
:::
...
| 你想做的事                        | 方式                                     |
| --------------------------------- | ---------------------------------------- |
| 处理 task —— 查看、关闭、说明结果 | **跟 agent 说话。没有对应的命令。**      |
| 操作 pipeline —— 查状态、立刻执行 | `/pm …`（Slack 上是 `pm …`），不经过模型 |

这个分工是刻意的。操作 pipeline 是机械性的，所以做成命令：精确、即时、不花钱。处理
task 是判断，所以走 agent —— 只有它读得懂「处理好了，那次部署修掉了」是什么意思。

## 处理一条 task

用你自己的话问 —— 这是**唯一**的方式，没有对应的 `/pm` 命令：
...
在 Slack 上，这些全部都不加开头的斜杠：`pm all`、`pm run`。
