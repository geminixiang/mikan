...
Slack auto-reply 现在是存储在 `slack.autoReply` 中的 conversation-level boolean，可通过 `/pi-auto-reply on|off` 修改。启用后，该 Slack channel 中未明确 address mikan 的 top-level human message 也会触发执行。旧标记文件、rules、top-level `autoReply` 与 `llm.autoReply` judge-model 设置仍保持退役并被忽略。
