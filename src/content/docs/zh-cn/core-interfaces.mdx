---
title: 核心接口
description: mikan 在平台、运行时、框架、会话、执行、vault 与主机各层之间受支持的边界。
---
...
| 级别             | 含义                                | 兼容性预期                           |
| ---------------- | ----------------------------------- | ------------------------------------ |
| Public           | 被 npm 使用方导入                   | 保持不变或有意进行版本管理           |
...
以下八个归属范围是长期的职责划分。所有者可以在其端口背后自由更改实现，但更改端口需要同时更新两侧以及下文列出的契约测试。这些范围描述的是代码归属，而非进程或权限隔离。
...
| **Office identity**    | `src/office/`                                                | `OfficeAddress`/`OfficeKey` 派生、`Workspace`/`Office` 布局值、办公室物化与工作区投影、注册表日志，以及旧版迁移 | 路径已预先计算的冻结 `Office` 值，以及按当前策略解析投影的入口；持久的原始 id ↔ 办公室映射 |
...
| **Harness/tools**      | `src/harness/`、`src/harness/tools/`                         | 模型轮次、执行解析、预算、工具 schema、skill、subagent 和主机能力调用                                           | `ConversationResponder` 输出与 `Executor` 操作；它不接收平台 SDK 对象                      |
| **Scheduled events**   | `src/events/`                                                | 事件 wire protocol、解析/构建、host store、文件名校验和 watcher 生命周期                                        | 向 harness/Web 提供 `EventStore`；向平台消息接口提供规范化事件                             |
| **Sandbox/workspace**  | `src/sandbox/`                                               | Executor 实现、mount 拓扑、runtime/主机路径上下文、托管容器生命周期和文件传输                                   | 一个声明了路径与凭证能力的具体 `Executor`                                                  |
| **Vault/web/security** | `src/vault/`、`src/adapters/web/`、`src/sandbox/identity.ts` | 凭证身份与注入策略、仅主机的 portal/token 界面、mount 冲突检查，以及安全敏感的授权决策                          | 交给 vault 策略的 actor/信任输入；凭证只交给选定的 executor                                |
| **Config/CLI**         | `src/settings/`、`src/cli/`、`src/adapters/commands/`        | 启动/flag 语法、设置持久化和命令清单                                                                            | 生效设置和确定性的命令清单                                                                 |

一个对话的框架实例与其他实例是上下文隔离的，但那不是文件系统或进程沙箱。同样地，沙箱 executor 改变的是工具在哪里运行以及它们接收哪些 mount；`host` 模式和 `full` 工作区投影并不是额外的安全保证。审查缝隙时请把这些关注点分开。
...
| Command inventory         | Config/CLI → Platform entry、Runtime/session 与 web/会话查看器              | `COMMAND_MANIFEST` 是唯一的清单。已识别的内置命令会被优先处理；未匹配的斜杠文本仍是代理提示词；`stop` 是魔法词，而非处理器。                                                                                                                                                                                                                                      |
...
| Settings and live runners | Config/CLI（命令和 admin 是适配器）→ 配置磁盘与 Runtime/session             | `applyConversationSettings` 在写入 LLM 更改**之前**清除缓存的 runner，并在忙碌时拒绝这两项操作。`applyConversationWorkspacePolicy` 对门禁策略更改采取同样做法，因为 mount 集合被固化在 runner 的环境中。全局 LLM 和门禁策略设置先写入，然后清除空闲 runner，并把忙碌的对话报告为过期。沙箱资源限制和 Slack 回复设置在使用时重新读取，不会清除 runner。            |
...
这些桥梁有意区分上下文隔离与沙箱化。新的所有者不得从路径推断凭证授权，不得从平台名称推断平台信任，也不得把共享的事件目录变成授权边界。
...
来源：`src/types.ts`（由 `src/adapters/index.ts` 重新导出）。
...
`createConversationRuntime(options)` 返回公开的 `ConversationRuntime` 编排接口。在 `src/runtime/` 内，`SessionLifecycle` 是 runner 资源的单一 authority：它拥有 per-session 串行化、materialization、lease、settlement、invalidation、disposal、闲置回收和关闭；runtime 编排器负责命令、stop/reset、rotation 与平台工作流。
...
每个会话范围的入口点都接受一个 `OfficeAddress`。不存在原始对话 id 的桥接；平台与 Admin 介面会明确携带带平台范围的地址。
...
并发不变式：对于一个办公室加一个会话密钥的调用是串行的；不同的会话密钥可以并发运行。每次使用 runner 都持有内部 lifecycle lease，因此设置失效、回收与关闭不会在操作途中 dispose 它。平台工具包仍按 runner 创建，绝不全局共享。

## 4. 命令接口

来源：`src/adapters/commands/manifest.ts`、`src/adapters/commands/types.ts` 与 `src/adapters/commands/registry.ts`。

`COMMAND_MANIFEST` 是面向平台的清单，用于派生斜杠形式和原生注册。`CommandHandler.tryHandle(context)` 是执行契约。处理器按顺序运行，第一个返回 `true` 的结果会消费该消息。

已识别的内置命令会在 runner 创建前处理。未匹配的斜杠前缀消息仍然是一个普通的代理提示词。`stop` 是一个平台接收魔法词，而非 `CommandHandler`；`session` 是唯一被接受的裸命令。

`CommandContext` 包含规范化的执行者/对话身份、响应与 bot 端口、命令文本、隐私状态以及 `CommandServices`。命令处理器不得深入平台 SDK 事件。

## 5. 框架接口

来源：`src/harness/index.ts`、`src/harness/runner.ts`、`src/sessions/session-store.ts`、`src/harness/models.ts` 与 `src/harness/types.ts`。

### `MikanAgentSession`

拥有模型轮次循环、消息持久化、重试、compaction、预算断路器、skill、有界 subagent 和中止行为。其外部有意义的操作是 prompt/run、subscribe、abort、会话重载、模型/thinking 选择和 disposal。

### `SessionStore`

负责 Pi 0.85 当前 v4 的仅追加会话 JSONL 树。持久化 header 使用 `v: 4` 和 `storageVersion: 1`；mikan 专属 metadata 存储为命名空间持久值 `mikan/metadata`。运行时只接受这一当前格式，因此旧版 mikan v3 和 Pi 0.84 代 v4 文件必须在 daemon 停止时通过 `mikan sessions migrate` 转换。兼容视图中的 `SessionHeader.version` 当前为 `4`（`CURRENT_SESSION_VERSION`）。会话条目可能分支；使用方必须使用 store/context 辅助函数，而不能假定文件是一份扁平的聊天记录。
...
subagent 始终以一个具名 profile 启动——按运行自由配置不是受支持的界面。`loadSubagentProfiles` 负责解析：`src/harness/subagent-profiles.ts` 中的内置 profile 是权威定义（`worker`、`software-engineer`、`devops-engineer`、`data-scientist`、`account-manager`、`business-development`、`creative-producer`、`ad-operations-specialist`、`summarizer`），而 `<workspace>/agents/<name>.md` 文件是对同名内置 profile 的**补丁**而非替换，因此运维人员可以只覆盖 `model:`，无需重述 tools 和提示词。名称与任何内置都不匹配的文件定义一个新 profile，且必须自带 `tools`。格式错误的文件会变成诊断信息并被跳过，绝不抛出异常。
...
来源：`src/harness/tools/types.ts`。

核心工具使用由 runner 提供的 executor 和主机服务。`src/events/index.ts` 的 `EventStore` 是事件文件的规范 CRUD 端口。无效的事件 JSON 仍以 `payload: null` 保持可列出，以便管理员诊断或删除它。
...
2. 执行解析器解析 Workspace projection，然后选择 actor/vault 身份。
3. `VaultManager` 解析 env 和 mount；collision 与 backend capability checks 验证完整 grant。
4. 单个 execution decision 返回 projection、runtime path context，以及只带本次 run 所需凭证的 concrete executor。
5. 提示词 memory/skills 与文件系统执行使用同一份 decision，不再分别重算 authorization。
...
来源：`src/adapters/web/server.ts` 与 portal 模块。这些路由支撑 mikan 捆绑的 UI；它们不是通用的公开 REST API。
