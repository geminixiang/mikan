...
Gondolin worker/gateway 路径是一个独立的 wire 协议，而非进程内的沙箱实现。它的契约位于 `gondolin-contract.ts`、`gondolin-worker-client.ts`、gateway/join 模块、inventory、placement 和 fleet 代码中。

稳定的概念是 runtime spec 和 handle 身份、worker 注册/心跳、租约式 placement、exec 会话、文件传输和运行时生命周期。Lease watermark 在故障转移期间隔离过期的 worker。Wire 变更必须进行版本管理，或在 controller、gateway 和 worker 之间兼容地部署。

有关详细的部署契约，参见 [Gondolin remote worker](/sandbox/gondolin-remote-worker/)。
