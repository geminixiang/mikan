---
title: 核心接口
description: mikan 在平台、运行时、框架、会话、执行、vault、package 与主机各层之间受支持的边界。
---
...
| **Config/CLI/package** | `src/config.ts`、`src/settings-mutation.ts`、`src/cli/`、`src/adapters/commands/`、`src/packages/` | 启动/flag 语法、设置持久化、命令清单、package 物化与范围解析                                        | 生效设置、确定性的命令清单，以及只读的 package skill mount            |
...
| Command inventory         | Config/CLI/package → Platform entry、Runtime/session 与 web/会话查看器      | `COMMAND_MANIFEST` 是唯一的清单。已识别的内置命令会被优先处理；未匹配的斜杠文本仍是代理提示词；`stop` 是魔法词，而非处理器。                                                                                                                                                                                                                                      |
...
2. 执行解析器只解析一次 Workspace projection 与 packages，然后选择 actor/vault 身份。
3. `VaultManager` 解析 env 和 mount；collision 与 backend capability checks 验证完整 grant。
4. 单个 execution decision 返回 projection、packages、runtime path context，以及只带本次 run 所需凭证的 concrete executor。
