...
分发是纯文本的：传入的消息文本必须以 `/pm` 开头。这在把 `/` 前缀文本当作普通消息投递的平台上有效——Telegram、Discord 和 GitHub 评论。Slack 客户端会拦截斜杠输入，只发送在 Slack App manifest 中注册过的命令（mikan 把这些路由到内置命令），因此扩展命令目前无法在 Slack 中以 `/pm` 调用。
