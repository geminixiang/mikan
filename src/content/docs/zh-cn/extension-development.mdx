...
分发是纯文本的：在 Telegram、Discord 和 GitHub 评论上，消息必须以 `/pm` 开头；在 Slack 上则不必。Slack 客户端会把每一行 `/` 开头的输入留给声明在 Slack App 里的命令，所以扩展的 `/pm` 根本离不开用户的电脑——而扩展也无法在那里声明，因为 `apps.manifest.update` 需要一个每天过期两次、且绑定在单一用户与单一 workspace 上的 app configuration token。因此 Slack 接受不带斜杠的名字：`pm list` 会分发，用户若粘贴 `/pm list` 也会。

这份宽松由 registry 收敛，不是由 parser。任何形状像名字的第一个词都会被解析出来；没有东西注册在那个名字下，文本就原封不动地交给 agent。它无法避免的是：一条 Slack 消息以某个已注册命令的名字开头、但说的是别的事——那会进到命令，然后拿回一行用法说明。响亮、即时、不花钱，相对于另一个选项：agent 用猜的，而且为一个看似合理的错误答案收费。取命令名字时，避开那些读起来像句子开头的词。
