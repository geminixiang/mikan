...
<CardGrid>
  ...
  <Card title="回复能力" icon="rocket">
    `ChatResponseContext` 封装回复、更新、typing/working 状态与文件上传。
  </Card>
</CardGrid>
...
6. 创建 `ChatResponseContext`，封装回复、更新、typing/working 状态与文件上传。
7. 把事件交给 `BotHandler`，也就是 `SessionRuntime`。
...
| 文件                        | 用途                                                                          |
| --------------------------- | ----------------------------------------------------------------------------- |
| `src/adapter.ts`            | 对外导出 `Bot`、`BotEvent`、`ChatMessage`、`ChatResponseContext` 等通用接口。 |
...
- `ChatResponseContext`：agent 回复时使用的通道。
