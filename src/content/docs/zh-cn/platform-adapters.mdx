...
它的目标是让 `src/runtime/*` 和 `src/harness/` 不需要知道每个平台的 SDK、thread 规则、消息更新 API 或文件下载方式。
...
2. 判断消息是否应该触发 mikan：DM、mention、thread reply。
...
通用类型由 `src/adapters/index.ts` 导出，实现类型放在 `src/types.ts` 与 `src/adapters/types.ts`。
...
Slash command 的注册与路由都派生自 `src/adapters/commands/manifest.ts`，而非各适配器各自的清单，因此一条新命令只需一处条目，
就能到达每个选择加入的平台。
...
| 文件                                   | 用途                                                                                                      |
| -------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `src/adapters/index.ts`                | 对外导出 `MessagingBot`、`ConversationEvent`、`ConversationMessage`、`ConversationResponder` 等通用接口。 |
...
如果新增平台，优先实现这些抽象，不要把平台 SDK 细节带进 `src/runtime/*` 或 `src/harness/`。
