...
:::caution[在 Slack 上要把斜線拿掉：`pm status`]
Slack 客戶端會把每一行 `/` 開頭的輸入留給宣告在 Slack App 裡的指令，所以 `/pm` 根本
離不開你的電腦 —— 而擴充功能也沒有辦法在那裡宣告。因此 Slack 接受不帶斜線的名字，
這頁的指令在 Slack 上就是這樣打。

其他地方 —— Telegram、Discord、GitHub comments —— 斜線不能省，而且省掉之後會安靜地
失敗：文字會進到 agent，而 agent 看到一個看似合理的請求就會猜，回你類似「無未完成
任務」的答案。看起來像對的，花掉一次模型呼叫，卻完全沒告訴你 pipeline 的狀況。

兩種情況下，指令的輸出都以 `agent-pm — <日期>` 開頭。沒看到那行，就代表你剛剛是在跟
agent 說話。
:::
...
| 操作 pipeline —— 查狀態、立刻執行 | `/pm …`（Slack 上是 `pm …`），不經過模型 |
...
在 Slack 上，這些全部都不加開頭的斜線：`pm all`、`pm run`。
