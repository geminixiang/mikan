...
分派是純文字的：進來的訊息文字必須以 `/pm` 開頭。這在會把 `/` 開頭的文字當成一般訊息送出的平台上可以運作——Telegram、Discord 與 GitHub comments。Slack 用戶端會攔截斜線輸入，只送出已註冊在 Slack App manifest 中的指令（mikan 會把那些路由到內建指令），因此目前在 Slack 中無法用 `/pm` 呼叫擴充功能指令。
