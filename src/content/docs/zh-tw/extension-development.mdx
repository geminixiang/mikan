...
使用 `api.registerTool(tool)` 註冊 `AgentTool`。請使用與 TypeBox 相容的參數結構描述，並回傳標準的文字／影像工具內容。如需具備型別的工具實作，請參閱完整的 [`agent-pm` 範例](https://github.com/geminixiang/mikan/tree/main/deploy/examples/extensions/agent-pm)。
...
分派是純文字的：在 Telegram、Discord 與 GitHub comments 上，訊息必須以 `/pm` 開頭；在 Slack 上則不必。Slack 用戶端會把每一行 `/` 開頭的輸入留給宣告在 Slack App 裡的指令，所以擴充功能的 `/pm` 根本離不開使用者的電腦——而擴充功能也無法在那裡宣告，因為 `apps.manifest.update` 需要一組每天過期兩次、且綁定在單一使用者與單一 workspace 上的 app configuration token。因此 Slack 接受不帶斜線的名字：`pm list` 會分派，使用者若貼上 `/pm list` 也會。

這份寬鬆由 registry 收斂，不是由 parser。任何長得像名字的第一個字都會被解析出來；沒有東西註冊在那個名字底下，文字就原封不動地交給 agent。它無法避免的是：一則 Slack 訊息以某個已註冊指令的名字開頭、但講的是別的事——那會進到指令，然後拿回一行使用說明。大聲、即時、不花錢，相對於另一個選項：agent 用猜的，而且為一個看似合理的錯誤答案收費。取指令名字時，避開那些讀起來像句子開頭的詞。
...
發佈時每個 `action_id` 都會加上 `ext:<slug>:…` namespace，因此互動只會交給此 extension 的 `onAction` handler，不會啟動 agent run。處理後應更新訊息以移除或停用按鈕。完整範例見 `deploy/examples/extensions/poll`。
...
當你需要同時具備工具、SQLite 持久化、排程、主動訊息或隨附技能時，請從 [`deploy/examples/extensions/agent-pm`](https://github.com/geminixiang/mikan/tree/main/deploy/examples/extensions/agent-pm) 開始。
