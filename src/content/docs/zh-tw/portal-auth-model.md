...
| 功能                 | 主要程式碼                                         |
| -------------------- | -------------------------------------------------- |
| Portal HTTP server   | `src/adapters/web/server.ts` 的 `startWebServer()` |
...
| 共用 token store     | `src/adapters/web/token-store.ts`                  |
| 共用 portal shell    | `src/adapters/web/portal-shell.ts`                 |
