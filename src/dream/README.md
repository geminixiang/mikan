...
- `prepareOfficeDream` runs inside the runtime's office maintenance barrier. It reads every regular session JSONL file in the office, compares each stable session UUID with the host-private checkpoint, and returns a bounded batch of entries after each session's `throughEntryId`. Oversized individual entries use an explicitly marked bounded head/tail representation. Backlogs advance over subsequent eligible sweeps instead of creating an unbounded model prompt.
