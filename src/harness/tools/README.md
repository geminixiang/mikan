...
- `index.ts`: Assembles core tools — pi-native `read`/`write`/`edit`/`bash` plus mikan's event, sandbox, attach, react, jev, and optional generate_image — exports the separately wired subagent tool factory, and merges optional `PlatformToolPack`s.
...
- `jev.ts`: Defines the `jev` tool (a `host-fn-tool`), which gives the model calibrated boolean/choice/score judgments from `harness/jev.ts`. `statePath` is read through the executor so large inputs are judged without entering the model's context; a missing `OPENROUTER_API_KEY` is a tool error, not a silent fallback.
