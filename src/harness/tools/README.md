...
- `index.ts`: Assembles core tools — pi-native `read`/`write`/`edit`/`bash` plus mikan's event, sandbox, attach, react, jev, jev_browser (through the sandbox Executor), and optional generate_image — merges optional `PlatformToolPack`s, and wraps the whole list with `withSecretRedaction`.
