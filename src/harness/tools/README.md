...
- `index.ts`: Assembles core tools — pi-native `read`/`write`/`edit`/`bash` plus mikan's event, sandbox, attach, react, jev, and optional generate_image — exports the separately wired subagent tool factory, merges optional `PlatformToolPack`s, and wraps the whole list with `withSecretRedaction`.
