...
- `source.ts`: The grammar. `parseSource` maps `<locator>[@<ref>][#<subpath>]` to a tagged `ParsedSource`; `sourceIdentity` decides package sameness for deduplication and scope precedence; `gitRepoPath` derives the transport-independent `<host>/<path>` that also becomes the clone's location on disk; `formatSource` renders the canonical string written back into settings.
- `materialize.ts`: The side effects. `materializeSource` clones or reconciles a git package under `<scope>/git/<host>/<owner>/<repo>` and returns the directory holding its files; its `offline` / `fetch` / `refresh` mode decides how far it may go to get them. `packageScopeDir` resolves the `global` / `conversations/<id>` root; `gitCloneDir` exposes the clone location.
