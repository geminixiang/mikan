...
- `chat-history-sync.ts`: `ChatHistorySync` — synchronizes platform `log.jsonl` into harness sessions and handles channel/thread bootstrap, rebuild, reset, and scope resolution. Also owns the platform-log reader: skips malformed lines and coalesces consecutive bot chunks sharing a `ts`, since one streamed response is logged in pieces. Command recognition is injected (`ChatHistorySyncOptions.isCommandText`) so the command inventory stays with its owner; Conversation runtime passes its instance to the runner as `CreateRunnerOptions.chatHistory`.
...
- `types.ts`: The module's shared interfaces — session header, session-key options, resolved scopes, log records, and the `ChatHistorySync` option/report bags. Importers take these types from `types.ts`, not from the implementing file.
